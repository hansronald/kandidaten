...
plot_scree = function(E, D, k){
  ...
  plot(E$values, ylab = "Egenvärden", xlab = "", ylim=c(0, max(E$values)), xlim = c(1,75),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, type = 'l')
  lines(diag(D), lty = 2)
  legend("topright", c("Otransformerade", "Roten ur"), lty = c(1,2),
         cex=1.5, bty = 'n')
...
adj_plot_with_evals = function(true_mat, est_mat, model = "", file_name,FN = FALSE, save_image = FALSE, show_title = TRUE){
  ...
  col = c("white","deepskyblue2", "darkorange")
